...
3. Interromper Execução Você está estritamente proibido de escrever scripts em tools/ até que:
...
1. A Regra do "Dados Primeiro" Antes de construir qualquer Ferramenta, você deve definir o Esquema de Dados em gemini.md.
* Como são os dados brutos de entrada?
* Como são os dados processados de saída? A codificação só começa após a confirmação do formato do "Payload".
...
2. Autocorreção (O Loop de Reparo) Quando uma Ferramenta falha ou ocorre um erro:
